...
test("eventId and eventRow are deterministic for a given time and suffix", () => {
  assert.equal(eventId(NOW, "req_x_0"), `evt_${NOW.getTime()}_req_x_0`);
...
test("appendEvent writes one events row with the nulls filled and a random suffix", async () => {
  ...
  const row = await appendEvent(deps, { request_id: "req_x", kind: "reviewed", actor: "agent", recommendation: "limit", revision: 2 });
  ...
  assert.equal(row.recommendation, "limit");
  ...
  assert.equal(again.request_id, null);

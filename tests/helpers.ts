...
/** The store tools over one Map: `store__<store>__<get|set|query>`, with the runtime's error envelope for a miss. `seedAt` preloads every seeded entitlement and the live requests as received then. */
export function fakeStore(seedAt?: string) {
  const store = new Map<string, Record<string, unknown>>();
  if (seedAt) {
    for (const e of seedRows(seedAt).entitlements) store.set(`entitlements:${e.entitlement_id}`, e);
    for (const r of REQUESTS) store.set(`requests:${r.request_id}`, requestRow(r, seedAt));
  }

...
test("removes every row in the four stores before seeding blind, then records the reset", async () => {
  const existing: Record<string, string[]> = { requests: ["req_a", "req_b"], entitlements: ["ent_1"], reviews: ["rev_1"], events: ["evt_1", "evt_2", "evt_3"] };
  ...
  assert.deepEqual(out.removed, { requests: 2, entitlements: 1, reviews: 1, events: 3 });
  assert.equal(out.seeded, seedRows(NOW).requests.length);
  ...
  assert.ok(names.lastIndexOf("store__events__remove") < names.indexOf("store__entitlements__set"), "every remove precedes the first seed write");
